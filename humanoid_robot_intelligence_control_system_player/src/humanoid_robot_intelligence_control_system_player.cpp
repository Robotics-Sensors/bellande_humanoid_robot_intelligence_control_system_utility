...
void play_sound_callback(const std_msgs::String::ConstPtr &msg) {
  ...
  case -1:
    ...
    break;
  case 0:
    execl("/usr/bin/madplay", "madplay",
          (g_sound_file_path + msg->data).c_str(), "-Q", (char *)0);
    break;
  default:
...
int main(int argc, char **argv) {
  ...
  if (g_sound_file_path != "" &&
      g_sound_file_path.compare(g_sound_file_path.size() - 1, 1, "/") != 0)
    g_sound_file_path += "/";

  ros::Subscriber play_mp3_sub =
      nh.subscribe("/play_sound_file", 10, &play_sound_callback);
  g_done_msg_pub = nh.advertise<std_msgs::String>("/humanoid_robot_intelligence_control_system/movement_done", 5);
