...
/*
 * humanoid_robot_intelligence_control_system_trajectory_calculator.cpp
 *
...

{
  ...
  poly_matrix << pow(mov_time, 3), pow(mov_time, 4), pow(mov_time, 5),
...
Eigen::MatrixXd calcMinimumJerkTraWithViaPoints(
    int via_num, double pos_start, double vel_start, double accel_start,
    Eigen::MatrixXd pos_via, Eigen::MatrixXd vel_via, Eigen::MatrixXd accel_via,
    double pos_end, double vel_end, double accel_end, double smp_time,
    Eigen::MatrixXd via_time, double mov_time)
/*
...

{
  ...
  for (int i = 1; i <= via_num; i++) {
    poly_vector.coeffRef(3 * i - 3, 0) =
        pos_via.coeff(i - 1, 0) - pos_start -
        vel_start * via_time.coeff(i - 1, 0) -
        (accel_start / 2) * pow(via_time.coeff(i - 1, 0), 2);

    poly_vector.coeffRef(3 * i - 2, 0) = vel_via.coeff(i - 1, 0) - vel_start -
                                         accel_start * via_time.coeff(i - 1, 0);

    poly_vector.coeffRef(3 * i - 1, 0) =
        accel_via.coeff(i - 1, 0) - accel_start;
  }

  poly_vector.coeffRef(3 * via_num, 0) = pos_end - pos_start -
                                         vel_start * mov_time -
  ...
  poly_vector.coeffRef(3 * via_num + 2, 0) = accel_end - accel_start;
  ...
  for (int i = 1; i <= via_num; i++) {
    ...
    poly_matrix_part1.coeffRef(3 * i - 2, 0) =
        3 * pow(via_time.coeff(i - 1, 0), 2);
    poly_matrix_part1.coeffRef(3 * i - 2, 1) =
        4 * pow(via_time.coeff(i - 1, 0), 3);
    poly_matrix_part1.coeffRef(3 * i - 2, 2) =
        5 * pow(via_time.coeff(i - 1, 0), 4);

    poly_matrix_part1.coeffRef(3 * i - 1, 0) = 6 * via_time.coeff(i - 1, 0);
    poly_matrix_part1.coeffRef(3 * i - 1, 1) =
        12 * pow(via_time.coeff(i - 1, 0), 2);
    poly_matrix_part1.coeffRef(3 * i - 1, 2) =
        20 * pow(via_time.coeff(i - 1, 0), 3);
  }

  Eigen::MatrixXd poly_matrix_part2 =
      Eigen::MatrixXd::Zero(3 * via_num, 3 * via_num);
  ...
  for (int i = 1; i <= via_num; i++) {
    for (int j = 1; j <= via_num; j++) {
      ...
      poly_matrix_part2.coeffRef(3 * j - 3, 3 * i - 3) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 3) / 6;
      poly_matrix_part2.coeffRef(3 * j - 3, 3 * i - 2) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 4) / 24;
      poly_matrix_part2.coeffRef(3 * j - 3, 3 * i - 1) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 5) / 120;

      poly_matrix_part2.coeffRef(3 * j - 2, 3 * i - 3) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 2) / 2;
      poly_matrix_part2.coeffRef(3 * j - 2, 3 * i - 2) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 3) / 6;
      poly_matrix_part2.coeffRef(3 * j - 2, 3 * i - 1) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 4) / 24;

      poly_matrix_part2.coeffRef(3 * j - 1, 3 * i - 3) =
          via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0);
      poly_matrix_part2.coeffRef(3 * j - 1, 3 * i - 2) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 2) / 2;
      poly_matrix_part2.coeffRef(3 * j - 1, 3 * i - 1) =
          pow((via_time.coeff(k - 1, 0) - via_time.coeff(i - 1, 0)), 3) / 6;
    }
  }
  ...
  for (int i = 1; i <= via_num; i++) {
    poly_matrix_part3.coeffRef(0, 3 * i) =
        pow(mov_time - via_time.coeff(i - 1, 0), 3) / 6;
    poly_matrix_part3.coeffRef(1, 3 * i) =
        pow(mov_time - via_time.coeff(i - 1, 0), 2) / 2;
    poly_matrix_part3.coeffRef(2, 3 * i) = mov_time - via_time.coeff(i - 1, 0);

    poly_matrix_part3.coeffRef(0, 3 * i + 1) =
        pow(mov_time - via_time.coeff(i - 1, 0), 4) / 24;
    poly_matrix_part3.coeffRef(1, 3 * i + 1) =
        pow(mov_time - via_time.coeff(i - 1, 0), 3) / 6;
    poly_matrix_part3.coeffRef(2, 3 * i + 1) =
        pow(mov_time - via_time.coeff(i - 1, 0), 2) / 2;

    poly_matrix_part3.coeffRef(0, 3 * i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 5) / 120;
    poly_matrix_part3.coeffRef(1, 3 * i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 4) / 24;
    poly_matrix_part3.coeffRef(2, 3 * i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 3) / 6;
  }

  Eigen::MatrixXd poly_matrix =
      Eigen::MatrixXd::Zero(3 * via_num + 3, 3 * via_num + 3);
  ...
  for (int i = 1; i <= via_num; i++)
    via_time_vector.coeffRef(i - 1, 0) =
        round(via_time.coeff(i - 1, 0) / smp_time) + 2;

  Eigen::MatrixXd minimum_jerk_tra_with_via_points =
      Eigen::MatrixXd::Zero(all_time_steps + 1, 1);

  for (int i = 1; i <= all_time_steps + 1; i++) {
    minimum_jerk_tra_with_via_points.coeffRef(i - 1, 0) =
        pos_start + vel_start * time_vector.coeff(i - 1, 0) +
  ...
  for (int i = 1; i <= via_num; i++) {
    for (int j = via_time_vector.coeff(i - 1, 0); j <= all_time_steps + 1;
         j++) {
      minimum_jerk_tra_with_via_points.coeffRef(j - 1, 0) =
          minimum_jerk_tra_with_via_points.coeff(j - 1, 0) +
          poly_coeff.coeff(3 * i, 0) *
              pow((time_vector.coeff(j - 1, 0) - via_time.coeff(i - 1, 0)), 3) /
              6 +
          poly_coeff.coeff(3 * i + 1, 0) *
              pow((time_vector.coeff(j - 1, 0) - via_time.coeff(i - 1, 0)), 4) /
              24 +
          poly_coeff.coeff(3 * i + 2, 0) *
              pow((time_vector.coeff(j - 1, 0) - via_time.coeff(i - 1, 0)), 5) /
              120;
    }
  }
...
Eigen::MatrixXd calcMinimumJerkTraWithViaPointsPosition(
    int via_num, double pos_start, double vel_start, double accel_start,
    Eigen::MatrixXd pos_via, double pos_end, double vel_end, double accel_end,
...

{
  ...
  poly_vector.coeffRef(via_num, 0) = pos_end - pos_start -
                                     vel_start * mov_time -
                                     (accel_start / 2) * pow(mov_time, 2);
  ...
  poly_vector.coeffRef(via_num + 2, 0) = accel_end - accel_start;
  ...
  for (i = 1; i <= via_num; i++) {
    poly_part3.coeffRef(0, i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 5) / 120;
    poly_part3.coeffRef(1, i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 4) / 24;
    poly_part3.coeffRef(2, i + 2) =
        pow(mov_time - via_time.coeff(i - 1, 0), 3) / 6;
  }
...
Eigen::MatrixXd calcArc3dTra(double smp_time, double mov_time,
                             ...
                             Eigen::MatrixXd start_point, double rotation_angle,
                             double cross_ratio) {
  int all_time_steps = int(round(mov_time / smp_time)) + 1;

  Eigen::MatrixXd angle_tra = calcMinimumJerkTra(0.0, 0.0, 0.0, rotation_angle,
  ...
  for (int step = 0; step < all_time_steps; step++) {
    ...
    weight_wedge << 0.0, -normal_vector.coeff(2, 0), normal_vector.coeff(1, 0),
    ...
    Eigen::MatrixXd rotation = identity + weight_wedge * sin(theta) +
                               weight_wedge * weight_wedge * (1 - cos(theta));
    double cross = cross_ratio * (1.0 - 2.0 * (abs(0.5 - (time / mov_time))));

    arc_tra.block(0, step, 3, 1) =
        (1 + cross) * (rotation * (start_point - center_point)) + center_point;
  }
