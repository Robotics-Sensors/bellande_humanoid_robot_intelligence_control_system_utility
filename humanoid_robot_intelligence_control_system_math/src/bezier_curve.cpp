...
#include "humanoid_robot_intelligence_control_system_math/bezier_curve.h"
...
Point2D BezierCurve::getPoint(double t) {
  ...
  for (unsigned int i = 1; i < (points_num - 1); i++) {
    point_at_t.x += control_points_[i].x * combination(points_num - 1, i) *
                    powDI(1 - t, points_num - 1 - i) * powDI(t, i);
    point_at_t.y += control_points_[i].y * combination(points_num - 1, i) *
                    powDI(1 - t, points_num - 1 - i) * powDI(t, i);
  }
