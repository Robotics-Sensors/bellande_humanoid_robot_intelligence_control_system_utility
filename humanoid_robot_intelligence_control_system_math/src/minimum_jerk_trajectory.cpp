...
#include "humanoid_robot_intelligence_control_system_math/minimum_jerk_trajectory.h"
...
MinimumJerk::MinimumJerk(double ini_time, double fin_time,
  ...
  if (fin_time_ > ini_time_) {
    ...
    time_mat << powDI(ini_time_, 5), powDI(ini_time_, 4), powDI(ini_time_, 3),
        powDI(ini_time_, 2), ini_time_, 1.0, 5.0 * powDI(ini_time_, 4),
        4.0 * powDI(ini_time_, 3), 3.0 * powDI(ini_time_, 2), 2.0 * ini_time_,
        1.0, 0.0, 20.0 * powDI(ini_time_, 3), 12.0 * powDI(ini_time_, 2),
        6.0 * ini_time_, 2.0, 0.0, 0.0, powDI(fin_time_, 5),
        powDI(fin_time_, 4), powDI(fin_time_, 3), powDI(fin_time_, 2),
        fin_time_, 1.0, 5.0 * powDI(fin_time_, 4), 4.0 * powDI(fin_time_, 3),
        3.0 * powDI(fin_time_, 2), 2.0 * fin_time_, 1.0, 0.0,
        20.0 * powDI(fin_time_, 3), 12.0 * powDI(fin_time_, 2), 6.0 * fin_time_,
        2.0, 0.0, 0.0;
  ...
  if (time >= fin_time_)
    ...
    for (int i = 0; i < number_of_joint_; i++) {
      Eigen::MatrixXd cur_pos =
          time_variables_ * position_coeff_.block(0, i, 6, 1);
  ...
  if (time >= fin_time_)
    ...
    for (int i = 0; i < number_of_joint_; i++) {
      Eigen::MatrixXd cur_vel =
          time_variables_ * velocity_coeff_.block(0, i, 6, 1);
  ...
  if (time >= fin_time_)
    ...
    for (int i = 0; i < number_of_joint_; i++) {
      Eigen::MatrixXd cur_acc =
          time_variables_ * acceleration_coeff_.block(0, i, 6, 1);
