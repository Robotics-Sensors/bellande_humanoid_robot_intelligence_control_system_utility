...
#include "humanoid_robot_intelligence_control_system_math/simple_trapezoidal_velocity_profile.h"
#include <iostream>
...
void SimpleTrapezoidalVelocityProfile::setVelocityBaseTrajectory(
    double init_pos, double final_pos, double acceleration,
    double max_velocity) {
  setVelocityBaseTrajectory(init_pos, final_pos, acceleration, acceleration,
                            max_velocity);
}

void SimpleTrapezoidalVelocityProfile::setVelocityBaseTrajectory(
    double init_pos, double final_pos, double acceleration, double deceleration,
    double max_velocity) {
  ...
  if (fabs(pos_diff) > 0.5 * fabs(max_velocity_) * (acc_time + dec_time)) {
    ...
    const_time_ =
        (fabs(pos_diff) - 0.5 * fabs(max_velocity_) * (acc_time + dec_time)) /
        fabs(max_velocity_);
    ...
    accel_time_ = sqrt(
        2 * fabs(pos_diff) * fabs(deceleration_) /
        (acceleration_ * acceleration_ + fabs(acceleration_ * deceleration_)));
  ...
  pos_coeff_const_section_.coeffRef(2, 0) =
      -0.5 * acceleration_ * accel_time_ * accel_time_ + initial_pos_;
  ...
  pos_coeff_decel_section_.coeffRef(2, 0) =
      0.5 * deceleration_ * total_time_ * total_time_ + final_pos_;
...
void SimpleTrapezoidalVelocityProfile::setTimeBaseTrajectory(
  ...
  setTimeBaseTrajectory(init_pos, final_pos, accel_time, accel_time,
                        total_time);
}

void SimpleTrapezoidalVelocityProfile::setTimeBaseTrajectory(
    double init_pos, double final_pos, double accel_time, double decel_time,
    double total_time) {
  initial_pos_ = init_pos;
  final_pos_ = final_pos;
...
double SimpleTrapezoidalVelocityProfile::getPosition() { return current_pos_; }

double SimpleTrapezoidalVelocityProfile::getVelocity() { return current_vel_; }

double SimpleTrapezoidalVelocityProfile::getAcceleration() {
  return current_acc_;
}

double SimpleTrapezoidalVelocityProfile::getTotalTime() { return total_time_; }
